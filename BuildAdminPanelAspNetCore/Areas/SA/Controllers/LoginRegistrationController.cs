            ...
            try
            {
                if (master.UserID != null && master.Password != null)
                {
                    ...
                    if (verifiedUserCredential != null)
                    {
                        ...
                        HttpContext.Session.SetString("UserID", verifiedUserCredential.UserID);
                        HttpContext.Session.SetString("enUserID", verifiedUserCredential.UserID);
                        HttpContext.Session.SetString("deUserID", Encryption.Decrypt(verifiedUserCredential.UserID));
                        HttpContext.Session.SetString("RoleID", verifiedUserCredential.RoleID);
                        HttpContext.Session.SetString("RoleName", verifiedUserCredential.RoleName);
                        HttpContext.Session.SetString("EmpID", verifiedUserCredential.EmpID.ToString());
                        HttpContext.Session.SetString("UserID", verifiedUserCredential.UserID);
                        if (!string.IsNullOrEmpty(verifiedUserCredential.EmpName))
                        {
                            HttpContext.Session.SetString("EmpName", verifiedUserCredential.EmpName);
                        }
                        else
                        {
                            HttpContext.Session.SetString("EmpName", Encryption.Decrypt(verifiedUserCredential.UserID));
                        }
                        ...
                        HttpContext.Session.SetString("IsLogged", MenuPopulate(verifiedUserCredential.UserID).ToString());
